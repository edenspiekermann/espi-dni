defmodule EspiDni.NotificationMessageControllerTest do
  ...
  setup do
    team = insert(:team)
    user = insert(:user)
    notification_message = insert(:notification_message, %{team: team})
    ...
    {:ok, conn: conn, team: team, notification_message: notification_message}
  end

  test "lists all entries on index", %{conn: conn} do
    conn = get conn, notification_message_path(conn, :index)
    assert html_response(conn, 200) =~ "Notification Messages"
  end
  ...
  test "shows chosen resource", %{conn: conn, notification_message: notification_message} do
    conn = get conn, notification_message_path(conn, :show, notification_message)
    assert html_response(conn, 200) =~ notification_message.text
  end
  ...
  test "renders form for editing chosen resource", %{conn: conn, notification_message: notification_message} do
  ...
  test "updates chosen resource and redirects when data is valid", %{conn: conn, notification_message: notification_message} do
    conn = put conn, notification_message_path(conn, :update, notification_message), notification_message: @valid_attrs
    assert redirected_to(conn) == notification_message_path(conn, :index)
    assert Repo.get_by(NotificationMessage, @valid_attrs)
  end

  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, notification_message: notification_message} do
    conn = put conn, notification_message_path(conn, :update, notification_message), notification_message: @invalid_attrs
    assert html_response(conn, 200) =~ "Edit notification message"
  end

  test "deletes chosen resource", %{conn: conn, notification_message: notification_message} do
